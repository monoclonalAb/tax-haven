...
class PickledData:
  ...
  def get_moving_average(self) -> int:
    return self.starfruit_sum // len(self.starfruit_values)
...
8.	Sunlight unit: Average sunlight per hour is 2500 units. The data/plot shows instantaneous rate of sunlight on any moment of the day
  ...
  def trade_coconut (self, state: TradingState) -> list[Order]:
    ...
    if (len(state.order_depths["COCONUT"].sell_orders) == 0 or len(state.order_depths["COCONUT"].buy_orders) == 0): return orders
    elif (len(state.order_depths["COCONUT_COUPON"].sell_orders) == 0 or len(state.order_depths["COCONUT_COUPON"].buy_orders) == 0): return orders
    ...
    for p in products:
      ...
      for price, coup_vol in obuy[p].items():
        vol_buy[p] += coup_vol 
        # if vol_buy[p] >= self.POSITION_LIMIT[p]/10:
        #   break
      for price, coup_vol in osell[p].items():
        vol_sell[p] += -coup_vol 
        # if vol_sell[p] >= self.POSITION_LIMIT[p]/10:
        #   break
    ...
    """
      ...
      if vol < 0:
        orders['COCONUT'].append(Order('COCONUT', worst_sell['COCONUT'], -vol))
    elif coco_benchmark < coup_benchmark and coco_benchmark > 0:
      vol = state.position.get('COCONUT', 0)
      i f vol > 0:
        orders['COCONUT'].append(Order('COCONUT', worst_buy['COCONUT'], -vol))
    """
    ...
    call = self.black_scholes_price(mid_price['COCONUT'], 10000, 248/252, 0, 0.1606393714, 'call')
    
    if (mid_price['COCONUT_COUPON'] > call):
      coup_vol = state.position.get('COCONUT_COUPON', 0) + position_limit['COCONUT_COUPON']
      if coup_vol > 0:
        orders['COCONUT_COUPON'].append(Order('COCONUT_COUPON', worst_buy['COCONUT_COUPON'], -coup_vol))
    else:
      coup_vol = position_limit['COCONUT_COUPON'] - state.position.get('COCONUT_COUPON', 0)
      if coup_vol > 0:
        orders['COCONUT_COUPON'].append(Order('COCONUT_COUPON', worst_sell['COCONUT_COUPON'], coup_vol))

    overfit = 33
    if (mid_price['COCONUT_COUPON'] > call + overfit):
    ...
    elif (mid_price['COCONUT_COUPON'] < call - overfit):
  ...
  def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
    ...
    traderData = jsonpickle.encode(pickled_data)
