...
class PickledData:
  def __init__(self, successful_trades: int = 0, number_of_timestamps: int = 0, conversions: int = 0, position_of_baskets: int = 0) -> None:
    ...
    self.position_of_baskets = position_of_baskets
  ...
  def change_position_of_baskets(self, position: int) -> None:
    self.position_of_baskets = position
    
  def return_position_of_baskets(self) -> int:
    return self.position_of_baskets
...
8.	Sunlight unit: Average sunlight per hour is 2500 units. The data/plot shows instantaneous rate of sunlight on any moment of the day
  ...
  def trade_basket (self, state: TradingState, gift_basket_position: dict) -> list[Order]: 
    orders = {'CHOCOLATE': [], 'ROSES': [], 'STRAWBERRIES': [], 'GIFT_BASKET': []}
    products = ['CHOCOLATE', 'ROSES', 'STRAWBERRIES', 'GIFT_BASKET']
    position_limit = {'CHOCOLATE': 250, 'ROSES': 60, 'STRAWBERRIES': 350, 'GIFT_BASKET': 60}
    ...
    if res_sell > trade_at: #selling the gift baskets
    ...
    elif res_sell < trade_at and res_sell > 0:
      position = state.position.get('GIFT_BASKET', 0)
      if position > 0:
        orders['GIFT_BASKET'].append(Order('GIFT_BASKET', best_buy['GIFT_BASKET'], -vol)) 
    elif res_buy < 0 and res_sell > -trade_at:
      position = state.position.get('GIFT_BASKET', 0)
      if position < 0:
        orders['GIFT_BASKET'].append(Order('GIFT_BASKET', best_sell['GIFT_BASKET'], -vol)) 
    elif res_buy < -trade_at: #buying the gift baskets
      vol = position_limit['GIFT_BASKET'] - state.position.get('GIFT_BASKET', 0)
      if vol > 0:
        orders['GIFT_BASKET'].append(Order('GIFT_BASKET', worst_sell['GIFT_BASKET'], vol))
    
    # self_position_limit = {'CHOCOLATE': 232, 'ROSES': 58, 'STRAWBERRIES': 348, 'GIFT_BASKET': 60}
    # self_positions = {'CHOCOLATE': state.position.get('CHOCOLATE', 0), 'ROSES': state.position.get('ROSES', 0), 'STRAWBERRIES': state.position.get('STRAWBERRIES', 0), 'GIFT_BASKET': state.position.get('GIFT_BASKET', 0)}
    # target_positions = {'CHOCOLATE': 4*gift_basket_position, 'ROSES': gift_basket_position, 'STRAWBERRIES': 6*gift_basket_position, 'GIFT_BASKET': gift_basket_position}
    
    # if res_sell > trade_at: #i am selling
    #   # vol = int(min(res_sell/trade_at, 1) * (self_positions['GIFT_BASKET'] + self_position_limit['GIFT_BASKET']))
    #   vol = self_positions['GIFT_BASKET'] + self_position_limit['GIFT_BASKET']
    #   if vol > 0:
    #     orders['GIFT_BASKET'].append(Order('GIFT_BASKET', worst_buy['GIFT_BASKET'], -vol))
    #     orders['CHOCOLATE'].append(Order('CHOCOLATE', worst_buy['CHOCOLATE'], -4*vol))
    #     orders['ROSES'].append(Order('ROSES', worst_buy['ROSES'], -vol))
    #     orders['STRAWBERRIES'].append(Order('STRAWBERRIES', worst_buy['STRAWBERRIES'], -6*vol))
    # elif res_buy < -trade_at:
    #   # vol =int(-min(res_buy/trade_at, 1) * (self_position_limit['GIFT_BASKET'] - self_positions['GIFT_BASKET']))
    #   vol = self_position_limit['GIFT_BASKET'] - self_positions['GIFT_BASKET']
    #   if vol > 0:
    #     orders['GIFT_BASKET'].append(Order('GIFT_BASKET', worst_sell['GIFT_BASKET'], vol))
    #     orders['CHOCOLATE'].append(Order('CHOCOLATE', worst_sell['CHOCOLATE'], 4*vol))
    #     orders['ROSES'].append(Order('ROSES', worst_sell['ROSES'], vol))
    #     orders['STRAWBERRIES'].append(Order('STRAWBERRIES', worst_sell['STRAWBERRIES'], 6*vol))
  ...
  def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
    ...
    gift_basket_position = pickled_data.return_position_of_baskets()
    basket_order, future_position = self.trade_basket(state, gift_basket_position)
    ...
    pickled_data.change_position_of_baskets(future_position)
