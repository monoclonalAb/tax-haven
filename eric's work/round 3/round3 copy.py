...
class PickledData:
  def __init__(self, successful_trades: int = 0, number_of_timestamps: int = 0, conversions: int = 0) -> None:
    ...
    self.moving_average_dict = {'STARFRUIT': [], 'CHOCOLATE': [], 'ROSES': [], 'STRAWBERRIES': []}
  ...
  def moving_average(self, product: str, value: int) -> None:
    self.moving_average_dict[product].append(value)
    if (len(self.moving_average_dict[product]) > 5): self.moving_average_dict[product].pop(0)
    return sum(self.moving_average_dict[product]) / len(self.moving_average_dict[product])
...
class Trader:
  ...
  def trade_starfruit (self, product: str, order_depth: OrderDepth, position: int , acceptable_bid: int, acceptable_ask: int, position_limit: int) -> list[Order]:
      orders: list[Order] = []
      ...
      for ask, vol in osell.items():
          if ((ask <= acceptable_bid) or ((position<0) and (ask == acceptable_bid+1))) and cpos < position_limit:
      ...
      for bid, vol in obuy.items():
        if ((bid >= acceptable_ask) or ((position>0) and (bid+1 == acceptable_ask))) and cpos > -position_limit:
...
8.	Sunlight unit: Average sunlight per hour is 2500 units. The data/plot shows instantaneous rate of sunlight on any moment of the day
  ...
  def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
    ...
    # moving_average = pickled_data.moving_average("STARFRUIT", average_starfruit_price)
    # # logger.print("moving average: " + str(moving_average))
    # starfruit_order = self.trade_starfruit("STARFRUIT", state.order_depths["STARFRUIT"], state.position.get("STARFRUIT", 0), moving_average-1, moving_average+1, 20)
    ...
    best_chocolate_bid_price = list(state.order_depths["CHOCOLATE"].buy_orders.keys())[0]
    best_chocolate_ask_price = list(state.order_depths["CHOCOLATE"].sell_orders.keys())[0]
    average_chocolate_price = (best_chocolate_ask_price + best_chocolate_bid_price) / 2
    chocolate_moving_average = pickled_data.moving_average("CHOCOLATE", average_chocolate_price)
    chocolate_order = self.trade_starfruit("CHOCOLATE", state.order_depths["CHOCOLATE"], state.position.get("CHOCOLATE", 0), chocolate_moving_average-1, chocolate_moving_average+1, 250)
    result["CHOCOLATE"] = chocolate_order
    
    best_roses_bid_price = list(state.order_depths["ROSES"].buy_orders.keys())[0]
    best_roses_ask_price = list(state.order_depths["ROSES"].sell_orders.keys())[0]
    average_roses_price = (best_roses_ask_price + best_roses_bid_price) / 2
    roses_moving_average = pickled_data.moving_average("ROSES", average_roses_price)
    roses_order = self.trade_starfruit("ROSES", state.order_depths["ROSES"], state.position.get("ROSES", 0), roses_moving_average-1, roses_moving_average+1, 60)
    result["ROSES"] = roses_order
    
    best_strawberries_bid_price = list(state.order_depths["STRAWBERRIES"].buy_orders.keys())[0]
    best_strawberries_ask_price = list(state.order_depths["STRAWBERRIES"].sell_orders.keys())[0]
    average_strawberries_price = (best_strawberries_ask_price + best_strawberries_bid_price) / 2
    strawberries_moving_average = pickled_data.moving_average("STRAWBERRIES", average_strawberries_price)
    strawberries_order = self.trade_starfruit("STRAWBERRIES", state.order_depths["STRAWBERRIES"], state.position.get("STRAWBERRIES", 0), strawberries_moving_average-1, strawberries_moving_average+1, 350)
    result["STRAWBERRIES"] = strawberries_order
